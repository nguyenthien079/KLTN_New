...
function parsePipeContent(pipeText) {
  return pipeText
    ...
    .map((line) => {
      ...
      return { entity_type: parts[1], start_offset: startOffset, end_offset: endOffset, surface_text: null, comment: null };
    })
    .filter(Boolean);
}
...
export default function DataListPage() {
  ...
  const openImportPicker = () => { folderInputRef.current?.click(); };
  ...
  const handleImportFolder = async (event) => {
    ...
    const pipeFiles = files.filter((f) => f.name.toLowerCase().endsWith('.pipe'));
    if (txtFiles.length === 0) { setErr('Folder đã chọn không có file .txt nào.'); setMsg(null); return; }
    ...
    try {
      ...
      for (const txtFile of txtFiles) {
        ...
        let annotations = enrichAnnotationsWithSurfaceText(pipeByKey.get(key) || [], txtContent);
  ...
  const applyQuickSelect = () => {
    ...
    if (!Number.isInteger(from) || !Number.isInteger(to) || from < 1 || to < 1) { setErr('Vui lòng nhập 2 số STT hợp lệ lớn hơn 0.'); setMsg(null); return; }
    const start = Math.min(from, to), end = Math.max(from, to);
    const selectedIds = items.slice(start - 1, end).map((item) => item.article_id);
    if (selectedIds.length === 0) { setErr('Khoảng STT không có file nào để chọn.'); setMsg(null); return; }
    setSelectedArticleIds((prev) => Array.from(new Set([...prev, ...selectedIds]))); setErr(null);
    setMsg(`Đã chọn nhanh ${selectedIds.length} file/text từ STT ${start} đến ${end}.`);
  };

  const toggleArticle = (id) => setSelectedArticleIds((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
  const toggleExpert = (id) => setSelectedExpertIds((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);

  const handleAssign = async () => {
    if (selectedArticleIds.length === 0) { setErr('Vui lòng chọn ít nhất một file/text để bàn giao.'); setMsg(null); return; }
    if (selectedExpertIds.length === 0) { setErr('Vui lòng chọn ít nhất một chuyên gia.'); setMsg(null); return; }
    setAssigning(true); setErr(null); setMsg(null);
    try {
      const initialAnnotations = {};
      selectedArticleIds.forEach((id) => { if (importedAnnotationsByArticleId[id]?.length) initialAnnotations[String(id)] = importedAnnotationsByArticleId[id]; });
      ...
      <div className="data-list-header">
        ...
        <div className="data-list-actions-bar">
          <button type="button" className="data-list-import-btn" onClick={openImportPicker} disabled={loading || importing}>
            {importing ? <><span className="data-list-spinner" /> Đang nhập...</> : '📂 Nhập '}
          </button>
          {importedFolderName && <span className="data-list-import-hint">📁 {importedFolderName}</span>}
        </div>
      </div>

      <input ref={folderInputRef} className="data-list-folder-input" type="file" multiple webkitdirectory="true" directory="true" accept=".txt,.pipe,text/plain" onChange={handleImportFolder} />
      ...
      <div className="data-list-layout">
        ...
        <div className="data-list-table-wrap">
          <div className="data-list-quick-select">
            <span className="data-list-quick-select-title">Chọn nhanh theo STT</span>
            <div className="data-list-quick-select-form">
              <input type="number" min="1" placeholder="Từ" value={quickFrom} onChange={(e) => setQuickFrom(e.target.value)} className="data-list-quick-select-input" />
              <span className="data-list-quick-select-sep">–</span>
              <input type="number" min="1" placeholder="Đến" value={quickTo} onChange={(e) => setQuickTo(e.target.value)} className="data-list-quick-select-input" />
                  ...
                  {visibleItems.map((item, index) => (
                    ...
                    >
                      ...
                      <td className="data-list-checkbox-cell" onClick={(e) => e.stopPropagation()}>
                        <input type="checkbox" checked={selectedArticleIds.includes(item.article_id)} onChange={() => toggleArticle(item.article_id)} />
                      </td>
                    </tr>
                  ))}
                </tbody>
        ...
        <div className="data-list-assignee-wrap">
          <div className="data-list-assignee-header">
            <h3 className="data-list-assignee-title">Chuyên gia nhận bàn giao</h3>
            {selectedExpertIds.length > 0 && <span className="data-list-expert-count">{selectedExpertIds.length} đã chọn</span>}
          </div>
          ...
          <div className="data-list-assignee-list">
            ...
            {experts.map((expert) => (
              <label key={expert.user_id} className={`data-list-assignee-item${selectedExpertIds.includes(expert.user_id) ? ' data-list-assignee-item--selected' : ''}`}>
                <input type="checkbox" checked={selectedExpertIds.includes(expert.user_id)} onChange={() => toggleExpert(expert.user_id)} />
                <div>
          ...
          <div className="data-list-assign-footer">
            <p className="data-list-helper">{selectedArticleIds.length} file · {selectedExpertIds.length} chuyên gia đã chọn</p>
            <button className="data-list-assign-btn" onClick={handleAssign} disabled={assigning || loading}>
              {assigning ? <><span className="data-list-spinner" /> Đang bàn giao...</> : 'Giao việc →'}
            </button>
          </div>
        </div>
      </div>
